...
B= True
DIR = "logs/presentation/save_rotation_run" if B else "logs/presentation/long run"
LOG_NAME = "Neat_Test"
FITNESS_HIDE_FILE = DIR + "/fitness/best_hide.npy"
FITNESS_SEEK_FILE = DIR + "/fitness/best_seek.npy"
CIAO_FILE = DIR+"/CIAO/"+ ("Epochs_80 Hider_2 Seeker_10 Map_map_empty_martin_save_run_1.npy" if B else "Epochs_80 Hider_2 Seeker_10 Map_map_empty.npy")
...
MAP_SIMPLE_NAME = "map_empty.txt"


ACTIONS = np.array([[-1,0], [1, 0], [0,-1], [0, 1]])
ACTIONS_SEEKER =  np.array([[-1,0], [-1,-1], [0,-1], [1,-1], [1, 0], [0, 1], [1,1], [-1,1]])
SCENT = 20
PENALTY_COEF = 1
PENALTY_LIFT= 5
...
CIAO_matrix = np.load(CIAO_FILE)
plt.ylabel("seeker generation")
plt.xlabel("hider generation")
plt.imshow(CIAO_matrix, cmap = matplotlib.colors.ListedColormap(["lime", "red"]))
plt.show()